...
int shmfd;
char* share_mem = 0;
/*客户链接数组，进程用客户连接的编号来索引这个数组，即可取得相关的客户链接
 * 数据*/
client_data* users = 0;
/*子进程和客户链接的映射关系表，用进程的PID来索引这个数组，即可取得该进程
 * 所处理的客户连接的编号*/
int* sub_process = 0;
/*当前客户数量*/
int user_count = 0;
	...
	while(!stop_child)
	{
		...
		for(int i=0; i<number; i++)
		{
			...
			if((sockfd == connfd) && (events[i].events & EPOLLIN))
			{
				 ...
				 * idx*BUFFER_SIZE处，长度为BUFFER_SIZE字节。因此，各个客户连接的
				 * 读缓存是共享的*/
...
int main(int argc, char* argv[])
{
	...
	while(!stop_server)
	{
		...
		for(int i=0; i<number; i++)
		{
			...
			if(sockfd == listenfd)
			{
				...
				else
				{
					...
					/*记录新的客户连接在数组users中的索引值，建立进程pid和该索引值之间
					 * 的映射关系*/
		...
		else if(events[i].events & EPOLLIN)
		{
			...
			{
				/*向除负责处理第child个客户连接的子进程之外的其它子进程发送消息，通知
				 * 它们有客户数据要写*/
