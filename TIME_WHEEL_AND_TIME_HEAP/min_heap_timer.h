...
class time_heap
{
	...
	/*构造函数之二，用已有数组来初始化堆*/
	time_heap(heap_timer** init_array,int size, int _capacity) throw(exception)
						:cur_size(size),capacity(_capacity)
	{
		if(capacity < size)
		{
			throw std::exception();
	...
	void add_timer(heap_timer* timer) throw(exception)
	{
		...
		if(cur_size >= capacity) /*如果当前容量不够，就将其扩大一倍*/
		{
			resize();
		}
		/*新插入了一个元素，当前堆大小增加1，hole是新建空穴的位置*/
		int hole = cur_size++;
		int parent = 0;
	...
	void del_timer(heap_timer* timer)
	{
		...
		/*仅仅将目标定时器的回调函数设置为空，即所谓的延迟销毁。这将节省真正删除
		 * 该定时器造成的开销，但这样容易使数组膨胀*/
		timer->cb_func = NULL;
	}
	/*获得堆顶部的定时器*/
