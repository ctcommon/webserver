/*服务器使用poll同时管理监听socket和连接socket。并且使用牺牲空间换取时间的
 * 策略来提高服务器性能*/
...
int main(int argc, char* argv[])
{
	...
	/*创建users数组，分配FD_LIMIT个client_data对象。可以预期：每个可能的socket
	 * 连接都可以获得一个这样的对象，并且socket的值可以直接用来索引（作为数组
	 * 的下标）socket连接对应的client_data对象。这是将socket和客户数据关联的简
	 * 单而高效的方式*/
