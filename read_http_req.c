...
/*主状态机的两种可能状态，分别表示：当前正在分析请求行，当前正在
 * 分析头部字段*/
enum CHECK_STATE {CHECK_STATE_REQUESTLINE = 0, CHECK_STATE_HEADER};

/*从状态机的三种可能状态：即行的读取状态，分别表示：读取到一个完整
 * 的行、行出错和行数据尚且不完整*/
enum LINE_STATE {LINE_OK = 0,LINE_BAD,LINE_OPEN};

/*服务器处理HTTP请求的结果：NO_REQUEST表示请求不完整，需要继续读取
 * 客户数据；GET_REQUEST表示获得了一个完整的客户请求；BAD_REQUEST
 * 表示客户请求有语法错误；FORBIDDEN_REQUEST表示客户对资源没有足够
 * 的访问的权限；INTERNAL_ERROR表示服务器内部错误；CLOSED_CONNECTION
 * 表示客户端已经关闭连接了*/
enum HTTP_CODE {NO_REQUEST,GET_REQUEST,BAD_REQUEST,FORBIDDEN_REQUEST,
								INTERNAL_ERROR,CLOSED_CONNECTION};

/*为了简化问题，没有给客户端发送一个完整的HTTP应答报文，而只是根据服务
 * 器的处理结果发送如下成果或失败信息*/
...
LINE_STATE parse_line(char* buffer, int& checked_index, int& read_index)
{
	 ...
	 * read_index指向buffer中客户数据尾部的下一个字节，buffer中第0~checked_
	 * index字节都已经分析完毕，第checked_index~(read_index)-1字节由下面的
	 * 循环挨个分析*/
	for(;checked_index < read_index; ++checked_index)
	{
		...
		if(temp = '\r')
		{
			/*如果'\r'字符碰巧是目前buffer中的最后一个已经被读入的客户数据，那
			 * 么这次分析没有读取到一个完整的行，返回LINE_OPEN表示还要继续读取
			 * 客户数据才能进一步分析*/
	...
	/*如果所有内荣都分析完毕还是没有遇到'\r'字符，则返回LINE_OPEN，表示还
	 * 需要继续读取客户数据才能进一步分析*/
	return LINE_OPEN;
}
...
{
	LINE_STATE linestatus = LINE_OK; /*记录当前行的读取状态*/
